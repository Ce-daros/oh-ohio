...
`npm run build` validates content and types, builds the client and server bundles, renders every public route to `dist/`, and checks the resulting HTML (per-route CSS, bundle isolation, sitemap and robots). `npm run preview` serves the build locally with the production redirect behavior. `npm run validate:content` checks the document graph without building the site. `npm test` runs the content-integrity and static-server test suites; the HTTP tests need a completed build in `dist/`. `npm run lint` runs ESLint over `src/` and `scripts/`; `npm run check:remote-media` HEAD-checks remotely hosted media. CI (`.github/workflows/ci.yml`) runs lint, validation, build, and tests in that order on every push and pull request.
