...
<template>
  <aside id="scene-selection" class="scene-selection" aria-label="Selected topic">
    <div class="selection-overview"><div class="selection-heading"><span class="selection-number">{{ String(position).padStart(2,'0') }}</span><p class="eyebrow">{{ selectedEntries.length }} STORIES</p></div><h2 aria-live="polite">{{ group.title }}</h2><p class="selected-summary">{{ getCollection(group.collectionId).dek }}</p><div class="chan-whisper"><span>Ohio-chan <span aria-hidden="true">♡</span></span><p>{{ guidance[world][group.id as keyof typeof guidance[typeof world]] }}</p></div></div>
