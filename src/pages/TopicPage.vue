...
<template>
  <main id="main-content" class="topic-page" tabindex="-1"><header class="topic-masthead"><RouterLink class="topic-back" to="/topics">← All collections</RouterLink><p class="eyebrow">A THREAD THROUGH OHIO</p><h1>{{ topic.title }}</h1><p class="topic-dek">{{ topic.dek }}</p></header><section class="topic-feature"><img v-if="cover" :src="cover.src" :alt="featured.kind === 'feature' ? featured.coverAlt : ''" width="1536" height="1024" fetchpriority="high" /><div><p class="eyebrow">START HERE</p><h2>{{ featured.title }}</h2><p>{{ featured.summary }}</p><RouterLink class="text-link" :to="featured.canonicalPath">Read the story <span>↗</span></RouterLink></div></section><section class="topic-reading"><div class="section-heading"><h2>Keep <em>following.</em></h2><p>{{ stories.length }} connected stories</p></div><div class="topic-stories"><ContentCard v-for="item in stories" :key="item.id" :content="item" :variant="item.kind === 'feature' ? 'story' : 'compact'" /></div></section></main>
</template>
<style scoped>
