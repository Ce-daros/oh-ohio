...
The homepage welcome portrait retains `public/ohio-chan-vtuber.webp` (1024 × 1536). Its `welcome-480.webp` and `welcome-768.webp` derivatives preserve the full composition and transparency; they were resized with Sharp at WebP quality 85 on 2026-09-25. Their sizes are 69,192 and 141,340 bytes, compared with the original 275,842 bytes. The hero supplies width descriptors and viewport-based `sizes`, keeps explicit dimensions and high fetch priority, and does not lazy-load the lead portrait. Narrow screens move the portrait below the identity line and omit the decorative signature stamp to keep the copy and entry button clear.
