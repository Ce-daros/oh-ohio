...
5. Run `node scripts/content-manifest.mjs`, `npm run validate:content`, and `npm run build`. The validator checks relationships, source and media references, block requirements, dates, files, legacy slugs, and manifest freshness. The build checks emitted route HTML and that every body remains in its own lazy client chunk.
